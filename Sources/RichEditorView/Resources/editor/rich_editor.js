...
RE.updatePlaceholder = function () {
  if (
    RE.editor.innerHTML.indexOf("img") !== -1 ||
    (RE.editor.textContent.length > 0 && RE.editor.innerHTML.length > 0)
  ) {
...
RE.setHeading = function (heading) {
  ...
  document.execCommand(
    "formatBlock",
    false,
    sel.tagName === `H${heading}` ? "<p>" : `<h${heading}>`
  );
};
...
RE.isCursorInTable = function () {
  return (
    document.querySelectorAll(":hover")[elements.length - 1] instanceof
